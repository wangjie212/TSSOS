...
d = 3
vsupp,vblocks,wsupp,wblocks,status = get_dynamic_sparsity(f, g, x, d, TS=["block","block"], SO=[1,1])
model = Model(optimizer_with_attributes(Mosek.Optimizer))
set_optimizer_attribute(model, MOI.Silent(), true)
v, vc, vb = add_poly!(model, x, vsupp)
w, wc, wb = add_poly!(model, x, wsupp)
Lv = v - sum(f .* differentiate(v, x))
info1 = add_psatz!(model, Lv, x, g, [], d, constrs="con1")
info2 = add_psatz!(model, w, x, g, [], d, constrs="con2")
info3 = add_psatz!(model, w-v-1, x, g, [], d, constrs="con3")
