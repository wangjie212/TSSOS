...
opt,sol,data = tssos(F, [G], x, 2, TS="block", QUIET=true)
...
r = 4
@time opt,sol,data = tssos(F, G, x, r, TS=false, QUIET=true)
@time opt,sol,data = tssos(F, G, x, r, TS="block", QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = tssos(data, TS="block", QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = tssos(F, G, x, r, TS="MD", QUIET=true, merge=true, md=1.1)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = tssos(data, TS="MD", QUIET=true, merge=true, md=1.1)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
...
@time opt,sol,data = tssos(data, TS="block", QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = tssos(F, [G], x, r, TS="MD", QUIET=true, merge=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = tssos(data, TS="MD", QUIET=true, merge=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = tssos(F, [G], x, r, TS=false, QUIET=false)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
...
G = [1 - sum(x.^2)]
@time opt,data = LinearPMI([-10, 1], F, G, x, 3, TS="block", QUIET=true)
@time opt,data = LinearPMI(data, TS="block", QUIET=true)
...
G = [4 - x[1]^2, 4 - x[2]^2]
d = 2
opt,sol,data = cs_tssos(F, G, x, d, TS=false, QUIET=true, Moment=true)
...
G = [4 - x[1]^2, 4 - x[2]^2]
d = 2
opt,sol,data = cs_tssos([y'*F*y; G; 1-sum(y.^2)], [x;y], d, numeq=1, TS=false, QUIET=true)

@polyvar x[1:3]
Q = [1/sqrt(2) -1/sqrt(3) 1/sqrt(6); 0 1/sqrt(3) 2/sqrt(6); 1/sqrt(2) 1/sqrt(3) -1/sqrt(6)]
F = (-x[1]^2 + x[2])*(Q[:,1]*Q[:,1]'+Q[:,2]*Q[:,2]') + (x[2]^2 + x[3]^2)*Q[:,3]*Q[:,3]'
G = [1 - x[1]^2 - x[2]^2, 1 - x[2]^2 - x[3]^2, -1 + x[2]^2 + x[3]^2]
opt,sol,data = cs_tssos(F, G, x, 2, TS=false, QUIET=true, Moment=true)
...
r = 4
@time opt,sol,data = tssos(F, G, x, r, TS=false, QUIET=true)
@time opt,sol,data = cs_tssos(F, G, x, r, TS=false, QUIET=true)
@time opt,sol,data = cs_tssos(F, G, x, r, TS="block", QUIET=true)
@time opt,sol,data = cs_tssos(F, G, x, r, TS="MD", QUIET=true, merge=true, md=1.5)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
...
@time opt,sol,data = cs_tssos(F, G, x, r, TS=false, QUIET=true)
@time opt,sol,data = cs_tssos(F, G, x, r, TS="block", QUIET=true)
@time opt,sol,data = cs_tssos(F, G, x, r, TS="MD", QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
...
@time opt,sol,data = tssos(F, G, x, 4, TS=false, QUIET=true)
...
@time opt,sol,data = cs_tssos(F, G, x, 4, TS=false, QUIET=true)
@time opt,sol,data = cs_tssos(F, G, x, 4, TS="block", QUIET=true)
@time opt,sol,data = cs_tssos(F, G, x, 4, TS="MD", QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
...
@time opt,sol,data = cs_tssos(F, G, x, 1, TS=false, QUIET=true)
...
@time opt,sol,data = cs_tssos(F, G, x, 2, TS="block", QUIET=true)
# println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = cs_tssos(F, G, x, 2, TS="MD", QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = cs_tssos(F, G, x, 2, TS=false, QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
@time opt,sol,data = tssos(F, G, x, 2, TS=false, QUIET=true)
println(maximum(maximum.([maximum.(data.blocksize[i]) for i = 1:data.cql])))
