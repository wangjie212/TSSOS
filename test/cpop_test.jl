...
opt,sol,data = complex_tssos(pop, z, 2, numeq=3, QUIET=true, TS="block", solution=true, Gram=true)
@test opt ≈ -0.4165019 atol = 1e-6
opt,sol,data = complex_tssos(data, QUIET=true, TS="block", solution=true, Gram=true)
...
opt,sol,data = complex_cs_tssos(pop, z, 2, numeq=1, QUIET=true, TS="block", solution=true, Gram=true)
...
opt,sol,data = complex_cs_tssos(pop, z, 2, numeq=3, QUIET=true, TS=false, solution=true, Gram=true)
@test opt ≈ -3.75 atol = 1e-6

@complex_polyvar z[1:2]
pop = [z[1]+z[2]+conj(z[1])+conj(z[2]), (1+im)*z[1]^2*conj(z[2])+(1-im)*z[2]*conj(z[1])^2, 1 - z'*z]
opt,sol,data = complex_cs_tssos(pop, z, 2, numeq=1, QUIET=true, TS="block", ConjugateBasis=true, solution=true, Gram=true)
@test opt ≈ -2.7423247 atol = 1e-6
opt,sol,data = complex_cs_tssos(pop, z, 2, numeq=1, QUIET=true, TS="block", ConjugateBasis=true, normality=2, solution=true, Gram=true)
@test opt ≈ -2.7423247 atol = 1e-6
opt,sol,data = complex_cs_tssos(pop, z, 2, numeq=1, QUIET=true, TS="block", solution=true, Gram=true)
@test opt ≈ -2.7423247 atol = 1e-6
opt,sol,data = complex_cs_tssos(data, QUIET=true, TS="block", solution=true, Gram=true)
@test opt ≈ -2.7423247 atol = 1e-6
...
opt,sol,data = complex_tssos(pop, x, 3, numeq=6, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ 1 atol = 1e-6
...
opt,sol,data = complex_tssos(pop, z, 3, numeq=6, TS="block", Gram=true, solution=true, QUIET=true)
@test opt ≈ 1 atol = 1e-6
...
opt,sol,data = complex_cs_tssos(cpop, z, 2, numeq=3, CS=false, TS="block", solution=true, QUIET=true, Gram=true)
...
opt,sol,data = complex_cs_tssos(cpop, z, 5, numeq=3, CS=false, TS="block", QUIET=true, solve=false)
opt,sol,data = complex_cs_tssos(data, TS="block", QUIET=true, solve=false)
opt,sol,data = complex_cs_tssos(data, TS="block", QUIET=true, Gram=true)
...
opt,sol,data = complex_cs_tssos([f; cons], z, 3, nb=N+1, CS=false, TS="block", ConjugateBasis=true, QUIET=true, Gram=true)
@test opt ≈ 1 atol = 1e-6
...
opt,data = complex_tssos_symmetry([f, 1-z[1]*conj(z[1])-z[2]*conj(z[2])], z, 2, G, TS=false, ConjugateBasis=false, QUIET=true)
@test opt ≈ -1.69150468 atol = 1e-6
opt,data = complex_tssos_symmetry([f, 1-z[1]*conj(z[1])-z[2]*conj(z[2])], z, 2, G, TS="block", ConjugateBasis=false, QUIET=true)
@test opt ≈ -2 atol = 1e-6
opt,data = complex_tssos_symmetry([f, 1-z[1]*conj(z[1])-z[2]*conj(z[2])], z, 2, G, numeq=1, TS="block", ConjugateBasis=true, QUIET=true)
@test opt ≈ -1.5 atol = 1e-6
opt,data = complex_tssos_symmetry(data, QUIET=true)
@test opt ≈ -1.5 atol = 1e-6
opt,data = complex_tssos_symmetry([f, 1-z[1]*conj(z[1]), 1-z[2]*conj(z[2])], z, 2, G, numeq=2, SymmetricConstraint=false, TS="block", ConjugateBasis=true, QUIET=true)
@test opt ≈ -1 atol = 1e-6
