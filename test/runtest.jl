...
opt,sol,data=tssos_first(f, x, newton=true, reducebasis=true, TS="block", solution=true)
...
h=x[2]^2+x[3]^2-1
pop=[f,g,h]
opt,sol,data=tssos_first(pop, x, 2, numeq=1, reducebasis=true, TS="block", solution=true)
opt,sol,data=tssos_higher!(data, TS="MD", solution=true)
...
d=2
opt,sol,data=cs_tssos_first(pop, x, d, numeq=1, TS="block", solution=true)
