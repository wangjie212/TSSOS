...
function blockcpop_first(pop,x,d;nb=0,numeq=0,reducebasis=false,TS="block",minimize=false,merge=false,QUIET=false,solve=true,extra_sos=false,solution=false,tol=1e-5)
    n=length(x)
    m=length(pop)-1
    dg=zeros(Int,m)
    ...
    if reducebasis==false
        fblocks,fcl,fblocksize,gblocks,gcl,gblocksize,ub,sizes,_=get_cblocks!(n,m,supp,ssupp,lt,fbasis,gbasis,nothing,nothing,nothing,nothing,nothing,nb=nb,TS=TS,minimize=minimize,QUIET=QUIET,merge=merge)
    else
        flag=1
        while flag==1
            fblocks,fcl,fblocksize,gblocks,gcl,gblocksize,ub,sizes,_=get_cblocks!(n,m,supp,ssupp,lt,fbasis,gbasis,nothing,nothing,nothing,nothing,nothing,nb=nb,reduce=true,TS=TS,minimize=minimize,QUIET=QUIET,merge=merge)
            gsupp=get_gsupp(n,m,lt,ssupp,gbasis,gblocks,gcl,gblocksize,nb=nb)
            tsupp=[ssupp[1] zeros(UInt8,n)]
            tsupp=[tsupp gsupp]
...
function blockcpop_higher!(data;reducebasis=false,TS="block",minimize=false,merge=false,QUIET=false,solve=true,extra_sos=false,solution=false,tol=1e-5)
    n=data.n
    nb=data.nb
    m=data.m
    x=data.x
    ...
    if reducebasis==false
       fbasis=data.fbasis
       fblocks,fcl,fblocksize,gblocks,gcl,gblocksize,ub,sizes,status=get_cblocks!(n,m,fsupp,ssupp,lt,fbasis,gbasis,gblocks,gcl,gblocksize,ub,sizes,nb=nb,TS=TS,minimize=minimize,QUIET=QUIET,merge=merge)
    else reducebasis==true
        fbasis=get_basis(n,d,nb=nb)
        flag=1
        while flag==1
              fblocks,fcl,fblocksize,gblocks,gcl,gblocksize,ub,sizes,status=get_cblocks!(n,m,fsupp,ssupp,lt,fbasis,gbasis,gblocks,gcl,gblocksize,ub,sizes,nb=nb,reduce=true,TS=TS,minimize=minimize,QUIET=QUIET,merge=merge)
              gsupp=get_gsupp(n,m,lt,ssupp,gbasis,gblocks,gcl,gblocksize,nb=nb)
              tsupp=[ssupp[1] zeros(UInt8,n)]
              tsupp=[tsupp gsupp]
              fbasis,flag=reducebasis!(n,tsupp,fbasis,fblocks,fcl,fblocksize,nb=nb)
...
function get_cgraph(n,tsupp,ssupp,lt,basis;nb=0,reduce=false)
    lb=size(basis,2)
    G=SimpleGraph(lb)
    if reduce==true||nb>0
        for i = 1:lb
            for j = i+1:lb
                ...
                while r<=lt
                    ...
                    if bfind(tsupp,size(tsupp,2),bi)!=0
                       break
                    else
                        r+=1
                    end
        ...
        for i = 1:lb
            for j = i+1:lb
                ...
                while r<=lt
                    bi=bin_add(basis[:,i],basis[:,j],nb)
                    bi=bin_add(bi,ssupp[:,r],nb)
                    if all(x->iseven(bi[x]), 1:n)||bfind(tsupp,size(tsupp,2),bi)!=0
                       break
                    else
                        r+=1
                    end
...
function get_cblocks!(n,m,supp,ssupp,lt,fbasis,gbasis,gblocks,gcl,gblocksize,ub,sizes;nb=0,reduce=false,TS="block",minimize=false,QUIET=true,merge=false)
    if gblocks==nothing
...
function blockcpop(n,m,ssupp,coe,lt,fbasis,gbasis,fblocks,fcl,fblocksize,gblocks,gcl,gblocksize;nb=0,numeq=0,QUIET=true,solve=true,solution=false,extra_sos=false)
    ...
    if solve==true
        ...
        bc=zeros(1,lsupp1)
        for i=1:lt[1]
            Locb=bfind(supp1,lsupp1,ssupp[1][:,i])
            if Locb==0
...
#             for j = i:flb
#                 bi=fbasis[:,i]+fbasis[:,j]
#                 if sum(Int[iseven(bi[k]) for k=1:n])==n||bfind(ofsupp,lfo,bi)!=0
#                    add_edge!(fG,i,j)
#                 end
#             end
#         end
...
#             for j = i+1:flb
#                 bi=fbasis[:,i]+fbasis[:,j]
#                 if sum(Int[iseven(bi[k]) for k=1:n])==n||bfind(osupp,lo,bi)!=0
#                     if alg=="greedy"
#                         cadd_edge!(G,i,j)
#                     else
#                         A[i,j]=1
